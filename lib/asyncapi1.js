...
const core = require('./cljs/widdershins.asyncapi1.core');
...
function derefAll(unrefd, derefd, cb) {
    if (unrefd.length == 0) {
        return cb(null, derefd);
    }
    else {
        var next = unrefd.pop();
        const derefOpts = {};
        pushDir(path.resolve(path.dirname(next.file)));
        return refParser.dereference(next.schema, derefOpts, function(err, result) {
          popDir();
          if (err) return cb(err, null);
          derefd.push(result);
          return derefAll(unrefd, derefd, cb);
        });
    }
}

function convertInner(rootSpec, subSpecs, options, callback) {
    let data = {};
    if (options.verbose) console.warn('starting deref',api.info.title);
    const derefOpts = {};
    pushDir(path.resolve(path.dirname(rootSpec.file)));
    return refParser.dereference(path.resolve(path.dirname(rootSpec.file)), rootSpec.schema, derefOpts, function(err, api) {
        popDir();
        ...
        return derefAll(subSpecs, [], function(err, subapis) {
            if (err) return callback(err, null);

            var apis = [api];
            for (var subapi of subapis)
              apis.push(subapi);
            data.resources = core.convert_to_toc(apis);
...
function convertMulti(api, subapis, options, callback) {
    let defaults = {};
    defaults.includes = [];
    defaults.search = true;
    defaults.theme = 'darkula';
    ...
    resolver.optionalResolve(options)
    .then(function(options){
        convertInner(api, subapis, options, callback);
    })
    .catch(function(ex){
        callback(ex);
    });
