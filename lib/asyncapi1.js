...
function convertToToc(sources) {
    ...
    for (let source of sources) {
      for (let t in source.topics) {
          ...
          for (var m in topic) {
              var message = topic[m];
              var tagName = source.info.title || 'Default';
...
function convertInner(api, subapis, options, callback) {
    let data = {};
    ...
    for (var subapi of subapis)
      apis.push(dereference(subapi,api,{verbose:options.verbose,$ref:'x-widdershins-oldRef'}));
    data.resources = core.convert_to_toc(apis); //convertToToc(apis);
...
function convertMulti(api, subapis, options, callback) {
    api = preProcessor(api);
    subapis = subapis.map(preProcessor);
