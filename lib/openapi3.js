...
function convertInner(spec, options, callback) {
    const derefOpts = {};
    if (options.verbose) console.warn('starting deref', spec);
    const scopeDir = path.resolve(path.dirname(spec.file))
    pushDir(scopeDir);
    return refParser.dereference(spec.schema, derefOpts, function(err, api) {
        popDir();
