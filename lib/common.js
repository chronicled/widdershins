...
function pushDir(path) {
  console.log("pushDir", path);
    process.chdir(path);
}

function deref(spec, cb) {
    const derefOpts = {};
    const scopeDir = path.resolve(path.dirname(spec.file))
    pushDir(scopeDir);
    return refParser.dereference(spec.schema, derefOpts, function(err, result) {
      popDir();
...
function doDerefAll(unrefd, derefd, cb) {
    if (unrefd.length == 0) {
        ...
        return refParser.dereference(next.schema, derefOpts, function(err, result) {
          ...
          if (err) return cb(err, null);
          derefd.push(result);
          return doDerefAll(unrefd, derefd, cb);
        });
    }
}

function derefAll(specs, cb) {
  return doDerefAll(specs, [], cb);
}
